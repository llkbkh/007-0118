...
class QVector
{
	public:
		...
		inline float Distance ( QVector& ToQVector )
		{
			return ( ToQVector - *this ).Length();
		}

		inline float Distance2D ( QVector& ToQVector )
		{
			return ( ToQVector - *this ).Length2D();
		}

		inline float Normalize ( void )
		{
			float RecipLength, QVectorLength;

			QVectorLength = Length();

			if ( QVectorLength != 0 )
			{
				RecipLength = 1 / QVectorLength;
